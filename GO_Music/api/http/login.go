...
func NewHandler(userManager *engine.UserManager, userRepo engine.UserRepository, jwtSecret string) *UserHandler {
    store := sessions.NewCookieStore([]byte(jwtSecret))
    return NewUserHandler(userManager, userRepo, store)
}
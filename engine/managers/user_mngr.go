...
type UserManager struct {
	*engine.BaseManager[int, domain.User, *domain.User]
...
func NewUserManager(
	...
	return &UserManager{
		BaseManager: engine.NewBaseManager[int, domain.User, *domain.User](repo, logger, txTimeout),
		repo:        repo,
		db:          db,
		auth:        auth,
	}
...
func (m *UserManager) SearchByNames(ctx context.Context, query string) ([]*domain.User, error) {
	...
	for _, user := range users {
		if user.Image == nil {
			user.Image = engine.DefaultImage // Подставляем изображение по умолчанию
		}
	}
...
// [RU] ensureUserImage загружает фото по умолчанию, если у пользователя его нет <--->
// [ENG] ensureUserImage load default profile image for user if he don't have any
func (m *UserManager) ensureUserImage(user *domain.User) {
	if user != nil && len(user.Image) == 0 {
		user.Image = engine.DefaultImage
	}
}

...
type StudyGroupManager struct {
	*engine.BaseManager[int, domain.StudyGroup, *domain.StudyGroup]
	db *sql.DB
}
...
func NewStudyGroupManager(
	...
	return &StudyGroupManager{
		BaseManager: engine.NewBaseManager[int, domain.StudyGroup, *domain.StudyGroup](repo, logger, txTimeout),
		db:          db,
	}
}
